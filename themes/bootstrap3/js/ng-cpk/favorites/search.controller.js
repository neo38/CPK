...
(function() {
    ...
    SearchController.$inject = [ '$window', '$scope', '$compile' ];
    ...
    function SearchController($window, $scope, $compile) {
        var favs = [],
        recordIsFav = [];
         ...
         */
        window.favorites.__favChanged = function(isNew, newFav) {
            if (!(newFav instanceof favorites.Favorite)) {
                return;
            }
        ...
        function addFavorite(rank) {
            ...
            fav.fromRecordSearch(rank);

            favs[rank] = fav;
            favorites.storage.add(favs[rank]).then(function() {
                switchAddRemoveSpanVisibility(rank);
                // Broadcast this event across all tabs
                // XXX favsBroadcaster.broadcastAdded(favs[rank]);
        ...
        function removeFavorite(rank) {
            ...
            favorites.storage.remove(id).then(function() {
                switchAddRemoveSpanVisibility(rank);
                // Broadcast this event across all tabs
                // XXX favsBroadcaster.broadcastRemoved(favs[rank]);
            }).catch(function(reason) {
                console.error(reason);
            });
        }
        ...
        function isFavorite(rank) {
            return new Promise(function(resolve, reject) {
                ...
                favorites.storage.has(recordId).then(
                    /**
                     * @param {Favorite} fav
                     */
                    function(fav) { resolve({ rank: rank, favorite: fav }); },
                    /**
                     * @param {String} reject
                     */
    ...
    function SearchDirective() {
        ...
        function linker(scope, elements, attrs) {
            ...
            if (typeof pubElements[rank] === 'undefined') {
                pubElements[rank] = { addFavBtn: undefined, remFavBtn: undefined };
            }
